...
export function validateBlock(block) {
    ...
    if (!block.blockName) {
        ...
        errors.push(invalidBlockNameError(block.blockName, getSuggestedBlockNames(block.blockName)));
    }
    // Check attrs
    if (!block.attrs) {
        ...
        errors.push(validationError('attrs', 'object', block.attrs, block.blockName));
    }
    ...
    if (block.innerBlocks) {
        if (!Array.isArray(block.innerBlocks)) {
            errors.push(validationError('innerBlocks', 'array', block.innerBlocks, block.blockName));
        }
        else {
 ...
 */
function getSuggestedBlockNames(input) {
    const suggestions = [];
    // Common mappings
    ...
    if (suggestions.length === 0) {
        suggestions.push('tpgb/tp-heading', 'tpgb/tp-pro-paragraph', 'tpgb/tp-button');
    }
    return suggestions;
}
/**
 ...
 */
export function validateAttributeType(value, expectedType, attributeName, blockName) {
 ...
 */
export function autoFixBlock(block) {
    const changes = [];
    const fixed = { ...block };
 ...
 */
export function validateAndFix(blocks) {
    const fixes_applied = [];
