...
export function registerTools(server, wpClient, schemaLoader) {
...
async function handleGetBlockSchemas(args, schemaLoader) {
    let schemas;
    if (args.block_names) {
...
async function handleCreateContent(args, wpClient) {
...
async function handleAnalyzeContent(args, wpClient, schemaLoader) {
    const postId = args.post_id;
    const includeSchemas = args.include_schemas ?? true;
    // Get content
    const content = await wpClient.getContent(postId);
    // Identify Nexter blocks
    const nexterBlocks = content.data.blocks.filter((b) => b.blockName?.startsWith('tpgb/'));
    const blockTypes = [...new Set(nexterBlocks.map((b) => b.blockName))];
    // Load schemas if requested
    let schemas = [];
    if (includeSchemas && blockTypes.length > 0) {
        schemas = await schemaLoader.getSchemas(blockTypes);
    }
    return {
        success: true,
        post_info: {
...
async function handleSearchContent(args, wpClient) {
...
// Helper functions
function modifyBlockAttributes(blocks, blockId, newAttrs) {
...
function insertBlock(blocks, position, newBlock) {
    const result = [...blocks];
